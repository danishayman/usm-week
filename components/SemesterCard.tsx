...
import { getIsoDateInTimeZone } from "@/lib/calendar/date";
...
// Every date shown is a USM date, so formatting is pinned to the campus zone
// rather than the viewer's. This also keeps the server and client renders
// identical, which is what makes hydration safe.
...
export default function SemesterCard({
  ...
  // Seeded from the server's clock so the first client render matches the
  // server HTML exactly; the effect below then takes over.
  ...
  const handleShare = async () => {
    ...
    const filename = `usm-week-${getIsoDateInTimeZone(now, TIME_ZONE)}.png`;
  ...
  useEffect(() => {
    // The seeded value comes from cached server HTML and can be minutes old,
    // so correct to the real client clock immediately rather than waiting a
    // tick for the interval.
  ...
  const semesterProgressLabel = getSessionLabel(
    info.termLabel,
